...
function updateCart() {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // NUEVO: Guardar carrito automáticamente para persistir la sesión
    // ═══════════════════════════════════════════════════════════════════════════
    saveCartState();
}
...
async function confirmPayment() {
    ...
    if (usuarioGoogle) {
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        // NUEVO: Enviar pedido a la pantalla de cocina
        // ═══════════════════════════════════════════════════════════════════════════
        const sentToKitchen = await sendToKitchen(sale);
        
        if (saved && sentToKitchen) {
            syncStatus.innerHTML = '✅ Guardado y enviado a cocina';
        } else if (saved) {
            syncStatus.innerHTML = '✅ Guardado (cocina offline)';
        } else {
...
function loadState() {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // NUEVO: Cargar carrito guardado para persistir la sesión
    // ═══════════════════════════════════════════════════════════════════════════
    const savedCart = localStorage.getItem('pos_cart');
    if (savedCart) {
        try { 
            cart = JSON.parse(savedCart); 
            console.log('🛒 Carrito restaurado:', cart.length, 'items');
        }
        catch (e) { cart = []; }
    }
}
...
function saveState() {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // NUEVO: Guardar carrito para persistir la sesión
    // ═══════════════════════════════════════════════════════════════════════════
    localStorage.setItem('pos_cart', JSON.stringify(cart));
}

/**
 * Guarda solo el carrito (llamado al modificar el carrito)
 */
...
// ╔════════════════════════════════════════════════════════════════════════════╗
// ║                    SECCIÓN 24: SISTEMA DE COCINA                            ║
// ╠════════════════════════════════════════════════════════════════════════════╣
// ║  Funciones para enviar pedidos a la pantalla de cocina                      ║
// ║  Los pedidos se guardan en Google Sheets para sincronización en tiempo real ║
// ╚════════════════════════════════════════════════════════════════════════════╝

/**
 * Envía un pedido a la pantalla de cocina
 * Guarda el pedido en la hoja "Pedidos_Cocina" de Google Sheets
 * @param {Object} sale - Objeto con los datos de la venta
 * @returns {boolean} - true si se guardó correctamente
 */
async function sendToKitchen(sale) {
    if (!usuarioGoogle) return false;
    
    try {
        // Preparar los items como JSON para la hoja
        const itemsForKitchen = sale.items.map(item => ({
            nombre: item.name,
            cantidad: item.quantity,
            acompañamiento: item.side || null
        }));
...
/**
 * Actualiza el estado de un pedido en la cocina
 * @param {number} orderNumber - Número del pedido
 * @param {string} newStatus - Nuevo estado (PENDIENTE, PREPARANDO, ENTREGADO)
 * @param {number} rowIndex - Índice de la fila en Google Sheets
 * @returns {boolean} - true si se actualizó correctamente
 */
async function updateKitchenOrderStatus(orderNumber, newStatus, rowIndex) {
    if (!usuarioGoogle) return false;
